...
export function TableContainer({
  ...
  emptyDescription = 'There are no incidents to display. Create a new incident to get started.',
    ...
    return (
      <div className={`bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden ${className}`}>
        <div className="flex flex-col items-center justify-center py-16 px-6">
          <div className="w-16 h-16 rounded-2xl bg-slate-100 flex items-center justify-center mb-4 transition-all duration-300">
...
export function TableRow({ children, onClick, className = '', disabled = false }: TableRowProps) {
  if (!onClick) {
    return (
      <tr className={`border-b border-slate-100 last:border-b-0 hover:bg-slate-50/50 transition-colors duration-150 ${className}`}>
        {children}
      </tr>
    );
  }
    ...
    <tr
      ...
      className={`
        ...
        ${disabled
          ...
          : 'hover:bg-brand-50/30 hover:shadow-sm'
        }
        ${className}
      `}
    >
...
export function Pagination({
  ...
  return (
    <div className={`flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/50 ${className}`}>
      {/* Info */}
      <div className="text-sm text-slate-600">
        Showing <span className="font-semibold text-slate-900">{startItem}</span> to{' '}
        <span className="font-semibold text-slate-900">{endItem}</span> of{' '}
        <span className="font-semibold text-slate-900">{totalItems}</span> incidents
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          className="p-2 rounded-lg text-slate-500 hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:-translate-x-0.5"
        >
...
export function SearchInput({ value, onChange, placeholder = 'Search incidents...', className = '' }: SearchInputProps) {
  return (
    <div className={`relative ${className}`}>
      ...
      <input
        ...
        className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-100 focus:border-brand-300 transition-all duration-200 hover:border-slate-300"
      />
      {value && (
        <button
          onClick={() => onChange('')}
          className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors p-1 hover:bg-slate-100 rounded"
        >
    ...
    >
      ...
      {onRemove && (
        <button
          ...
          className="ml-1 hover:text-red-600 transition-colors p-0.5 hover:bg-red-50 rounded"
        >
...
export function ActiveFiltersBar({ filters, onClearAll, className = '' }: ActiveFiltersBarProps) {
  ...
  return (
    <div className={`flex items-center gap-2 flex-wrap ${className}`}>
      ...
      <button
        onClick={onClearAll}
        className="text-xs text-slate-500 hover:text-red-600 font-medium transition-colors ml-2"
      >
        Clear all
      </button>
    </div>
